...
(function () {
                  ...
                  <div style="display:flex;gap:0;border:1px solid var(--c-border,#e5e7eb);
                    ...
                    <button id="taskScopeAll" onclick="Teacher._setTaskScope('all')"
                            style="flex:1;padding:.4375rem .5rem;font-size:.75rem;font-weight:600;
                                   ...
                                   background:var(--brand,#3b5bdb);color:#fff;">All Students</button>
  ...
  async function saveTasksConfig() {
    ...
    if (dates.length === 0) { UI.toast('Please add at least one date.', 'warning'); return; }

    // Build dateSubjects map — empty array = no restriction for that date
    const dateSubjects = {};
  ...
  function _loadTasksManager() {
    ...
    const unsubStudents = Db().collection('students').orderBy('name').onSnapshot(snap => {
      ...
      if (_taskScope === 'student') {
        // Rebuild date rows so subject lists reflect the now-loaded student cache
        _onTaskTargetChange();
      }
    });
