...
(function () {
  ...
  const SpeechEngine = {
    ...
    _cleanText(text) {
      if (!text) return '';
      return text
        .replace(/\\\((.*?)\\\)/g, ' $1 ')          // KaTeX inline
        .replace(/\\\[(.*?)\\\]/g, ' $1 ')          // KaTeX block
        .replace(/\$(.*?)\$/g, ' $1 ')              // Dollar math
    ...
    startListening() {
      if (!this.recognition) {
        if (window.UI) UI.toast('Voice commands require Chrome/Edge/Safari.', 'warning');
        return;
      }
      ...
      try {
        ...
        if (window.UI) UI.toast('Listening… Say: Next, Back, Select A, Read, or Submit.', 'info', 2500);
      } catch (err) {
    ...
    _handleCommand(cmd) {
      ...
      if (window.Exam) {
        if (cmd.includes('next') || cmd.includes('forward')) {
        ...
        } else if (cmd.includes('submit') || cmd.includes('finish') || cmd.includes('end')) {
          ...
          if (match) {
            ...
            const labels = document.querySelectorAll('.option-label, .cbt-option');
    ...
    injectExamButtons() {
      ...
      const header = document.querySelector('.exam-header-sticky, .cbt-exam-header, .glass');
      if (!header) return;
      ...
      ttsBtn.onclick = () => this.readCurrentQuestion();
    ...
    readCurrentQuestion() {
      if (!window.AppState || !AppState.exam || AppState.exam.step !== 'exam') {
        if (window.UI) UI.toast('No active question.', 'warning');
        return;
      }

      const qEl = document.querySelector('.vtx-question-text, #questionText, .question-text, .vtx-question-wrap h3, .vtx-question-wrap p:first-of-type, [class*="question"] h3');
      const opts = [...document.querySelectorAll('.option-label, .cbt-option')];

      let text = qEl ? qEl.textContent.trim() : 'Question';
      if (opts.length) {
        text += '. Options: ' + opts.map((el, i) => {
          const letter = String.fromCharCode(65 + i);
          return letter + ': ' + el.textContent.trim();
        }).join('. ');
      }
